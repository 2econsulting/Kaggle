...
# save param
file_param = paste0("PARAM_LGB",round(output$cvpredict_score,3)*10^3,".Rda")
saveRDS(optimalParams$scores, file.path(path_output, file_param))
cat(">> best params saved! \n")

# save ztable
file_ztable = paste0("ZTABLE_LGB",round(output$cvpredict_score,3)*10^3,".csv")
fwrite(data.frame(ztable=output$ztable), file.path(path_output, file_ztable))
cat(">> ztable saved! \n")

# save submit
file_pred = paste0("SUBMIT_LGB",round(output$cvpredict_score,3)*10^3,".csv")
