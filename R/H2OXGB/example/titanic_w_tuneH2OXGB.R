...
xgb_4_catencode <- tuneH2OXGB(
  data = train, 
  y = "Survived", 
  max_models = 100, 
  max_runtime_secs = 60*60*24,
  grid_id = "xgb_4_catencode",
  params=list(
    categorical_encoding = c("Enum","OneHotInternal","OneHotExplicit","Binary","SortByResponse")
  )
)
categorical_encoding = xgb_4_catencode@summary_table$categorical_encoding[1]
cat(">> best categorical_encoding :", categorical_encoding, "\n")

# step[3] : find range max_depth
xgb_4_maxdepth <- tuneH2OXGB(
  data = train, 
  y = "Survived", 
  max_models = 100, 
  max_runtime_secs = 60*60*24,
  grid_id = "xgb_4_maxdepth",
  params=list(
    categorical_encoding = categorical_encoding,
    max_depth = c(6, 2, 3, 4, 5, 7, 9, 11, 13)
  )
)
TOP1_maxdepth = xgb_4_maxdepth@summary_table$max_depth[1]
TOP2_maxdepth = xgb_4_maxdepth@summary_table$max_depth[2]
TOP3_maxdepth = xgb_4_maxdepth@summary_table$max_depth[3]
...
bst <- h2o.getModel(xgb@summary_table$model_ids[1])
...
make_submit(pred,"R_TEST_H2OXGB_TUNE_PRED.csv")

# h2o_xgb_cv_predict
output = h2o_xgb_cv_predict(
  data = train, 
  test = test,
  k = 12,
  y = "Survived",
...
make_submit(pred, name = "R_TEST_H2OXGB_TUNE_CVPRED")
p1 <- ifelse(rowMeans(output$p1)>0.5,1,0)
make_submit(p1, name = "R_TEST_H2OXGB_TUNE_CVP1") # 0.81339
